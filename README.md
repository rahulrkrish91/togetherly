...
### Gradle error: `Could not set unknown property 'classifier'` in `expo-constants`

This usually means Android is compiling an older `expo-constants` package that is not compatible with the Gradle version generated by Expo SDK 52. This project pins `expo-constants` to the SDK 52-compatible `~17.0.8` range.

Run this reset sequence from the project root:
...
If you do not want to regenerate the native Android project, install/fix dependencies and clean Gradle instead:
...
### Gradle error: `Unsupported class file major version 65`

`Unsupported class file major version 65` means the Android Gradle build is running with Java 21 bytecode while the generated Gradle/Android plugin stack expects Java 17. Use JDK 17 for local Android builds.
...
On Linux, install/use JDK 17 and rebuild:
...
java -version
cd /path/to/togetherly
rm -rf android/.gradle ~/.gradle/caches/8.0.1
npx expo prebuild --clean --platform android
cd android
./gradlew clean assembleDebug
```

On macOS, point Gradle at JDK 17 before building:
...
If you use SDKMAN, switch to Java 17 with:
...
This repository includes `.java-version` set to `17` so Java version managers such as `jenv`, `asdf`, or SDKMAN-based workflows can pick the expected JDK for Android builds.
